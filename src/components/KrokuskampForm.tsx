...
const categorieOptions = [
    { value: 'U8-U9',   label: 'U8 – U9' },
    { value: 'U10-U11', label: 'U10 – U11' },
    { value: 'U12-U13', label: 'U12 – U13' },
    { value: 'U14+',    label: 'U14+' },
];

const uitpasOptions = [
    { value: 'non', label: 'NEE (Non)' },
    { value: 'oui', label: 'JA (Oui)' },
];

export function KrokuskampForm() {
    ...
    const validate = (): boolean => {
        ...
        if (!form.postePied) e.postePied = 'Champ obligatoire';
        if (!form.semaine) e.semaine = 'Veuillez confirmer la semaine';
        if (!form.uitpas) e.uitpas = 'Champ obligatoire';
        if (!form.uitpasNumero) e.uitpasNumero = 'Indiquer 0 si vous n\'avez pas de numéro';
        ...
        <section className="w-full bg-[#EFF2F6] py-16 px-4 sm:px-6">
            <div className="max-w-2xl mx-auto">
                ...
                <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-5">
                    ...
                    <Field label="Date de naissance" sublabel="/ Geboorte datum">
                        <input
                            type="date"
                    ...
                    {/* Poste & Pied */}
                    <Field label="Poste & Pied de prédilection" sublabel="/ Positie op het veld & Beste voet" required>
                        <input
                            type="text"
                            ...
                            className={inputClass}
                        />
                        {errors.postePied && <p className="inter text-xs text-red-500">{errors.postePied}</p>}
                    </Field>
                    ...
                    <Field label="Choix de la semaine de stage" sublabel="/ Keuze van de trainingsweek" required>
                        <div className="bg-white border border-black/15 rounded-xl px-4 py-4">
                            <label className="flex items-center gap-3 cursor-pointer">
                                <input
                                    type="checkbox"
                                    checked={form.semaine}
                                    onChange={set('semaine')}
                                    className="w-4 h-4 accent-orange-500 shrink-0"
                                />
                                <span className="inter text-sm text-black">Van 08/02 tot 12/02/2027</span>
                            </label>
                            {errors.semaine && <p className="inter text-xs text-red-500 mt-2">{errors.semaine}</p>}
                        </div>
                    </Field>

                    {/* UITPAS */}
                    <Field label="Uitpas Kansen Tarief" required>
                        <div className="bg-white border border-black/15 rounded-xl px-4 py-4">
                            <RadioGroup
                                name="uitpas"
                                options={uitpasOptions}
                                value={form.uitpas}
                                onChange={setRadio('uitpas')}
                                error={errors.uitpas}
                            />
                        </div>
                    </Field>
                    ...
                    {/* UITPAS Numéro */}
                    <Field label="Numéro Uitpas Kansen Tarief" sublabel="/ Nummer" required>
                        <input
                            type="text"
                            placeholder="Indiquer 0 si vous n'avez pas de numéro"
                            value={form.uitpasNumero}
                            onChange={set('uitpasNumero')}
                            className={inputClass}
                        />
                        {errors.uitpasNumero && <p className="inter text-xs text-red-500">{errors.uitpasNumero}</p>}
                    </Field>
                    ...
                    <LegalBlock title="Conditions d'inscription / Voorwaarden van inschrijving">
                        ...
                        <ul className="list-disc list-inside space-y-1.5">
                            ...
                            <li>
                                ...
                                <ul className="list-disc list-inside ml-4 mt-1 space-y-0.5">
                                    ...
                                    <li><strong>Communication :</strong> <code className="bg-black/5 px-1 rounded text-xs">ESOANINOVE CARNAVAL VOETBALKamp2027</code></li>
                                </ul>
                            </li>
                            <li>
                        ...
                        <ul className="list-disc list-inside space-y-1.5">
                            ...
                            <li>
                                ...
                                <ul className="list-disc list-inside ml-4 mt-1 space-y-0.5">
                                    <li><strong>IBAN :</strong> BE54 0689 3810 3797</li>
                                    <li><strong>Mededeling :</strong> <code className="bg-black/5 px-1 rounded text-xs">ESOANINOVE CARNAVAL VOETBALKamp2027</code></li>
                                </ul>
                            </li>
                            <li>
